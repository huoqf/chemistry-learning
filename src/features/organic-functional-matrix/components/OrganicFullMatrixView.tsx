...
export const OrganicFullMatrixView: React.FC<OrganicFullMatrixViewProps> = ({
  ...
  const filteredGroups = useMemo(() => {
    return FUNCTIONAL_GROUPS.filter((g) => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!matchName && !matchFormula && !matchPhenomenon && !matchNotes && !matchReagent) {
          return false
        }
      }
  ...
  return (
    <div className="w-full h-full p-3.5 overflow-y-auto overflow-x-hidden space-y-3.5 text-slate-800 bg-slate-50/50">
      ...
      {/* 3. 核心：自适应无横向滚动全景反应矩阵大表 (文字字号与清晰度全面升级) */}
      <div className="p-3.5 bg-white rounded-2xl border border-slate-200 shadow-2xs space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
          ...
          <table className="w-full table-fixed text-left border-collapse divide-y divide-slate-200">
            <thead className="bg-slate-50 text-slate-800 font-bold select-none text-xs">
              <tr>
                ...
                <th className="w-[8%] py-2.5 px-1 text-center bg-blue-50/80 text-blue-950">
                  Na
                </th>
                <th className="w-[9%] py-2.5 px-1 text-center bg-pink-50/80 text-pink-950">
                  NaOH
                </th>
                <th className="w-[9%] py-2.5 px-1 text-center bg-purple-50/80 text-purple-950">
                  NaHCO₃
                </th>
                <th className="w-[9%] py-2.5 px-1 text-center bg-indigo-50/80 text-indigo-950">
                  Na₂CO₃
                </th>
                <th className="w-[9%] py-2.5 px-1 text-center bg-orange-50/80 text-orange-950">
                  浓 Br₂
                </th>
                <th className="w-[8%] py-2.5 px-1 text-center bg-emerald-50/80 text-emerald-950">
                  H₂
                </th>
                      ...
                      <td className="py-2.5 px-1 text-center font-mono">
                        {g.consumptions.NaOH > 0 ? (
                          <div className="inline-flex flex-col items-center">
                            ...
                            {g.consumptions.NaOH === 2 && (
                              <span className="text-[10px] text-rose-700 font-extrabold mt-0.5">
                                酚酯双耗!
                              </span>
                            )}
                          </div>
