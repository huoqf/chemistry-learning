    ...
    <div className="w-full h-full p-2 flex flex-col gap-1.5 overflow-hidden text-slate-800 select-none">
      ...
      <div className="shrink-0 px-2.5 py-1.5 bg-white rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 overflow-hidden">
          ...
          {/* 醒目全屏大表按钮（采用项目标准 Primary Chemistry Blue 色阶） */}
          <button
            type="button"
            onClick={() => setIsFullscreenMatrixOpen(true)}
            className="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 active:bg-blue-800 active:scale-95 text-white text-xs font-bold flex items-center gap-1.5 shadow-2xs hover:shadow-xs transition-all cursor-pointer shrink-0"
            title="点击进入全屏沉浸探究大表模式"
            aria-label="全屏全景大表"
          >
