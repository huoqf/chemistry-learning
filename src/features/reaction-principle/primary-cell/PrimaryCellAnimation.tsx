import { useAnimationViewport, useSceneScale } from '@/hooks'
import { CANVAS_PRESETS, CHEMISTRY_COLORS } from '@/theme'
import { AnimationSvgCanvas } from '@/components/Layout'
import { BaseChart, ChartLine, ChartCursor } from '@/components/Chart'
import { useAnimationStore } from '@/stores'
import { useShallow } from 'zustand/react/shallow'
import { usePrimaryCellChemistry } from './hooks/usePrimaryCellChemistry'
import { PrimaryCellScene } from './components/PrimaryCellScene'
import type { LoadType } from '@/components/Chemistry'

export default function PrimaryCellAnimation() {
  // 1. 准确订阅 Store 状态
  const { params, time } = useAnimationStore(
    useShallow((s) => ({ params: s.params, time: s.time }))
  )

  // 2. 布局 Viewport (splitHw 280px 装置区 + 560px 图表区)
  const { containerRef, canvasSize, vp } = useAnimationViewport({
    preset: CANVAS_PRESETS.splitHw,
  })

  // 3. 参数提炼
  const cellType = Number(params.cellType ?? 0)
  const electrolyteType = Number(params.electrolyteType ?? 0)
  const loadType = Number(params.loadType ?? 0) as LoadType
  const current = Number(params.current ?? 1.5)

  // 4. 化学计算
  const chemistry = usePrimaryCellChemistry({
    cellType,
    electrolyteType,
    current,
    time,
  })

  // 5. 缩放与比例尺 (280x650 design)
  const sceneScale = useSceneScale({
    vp,
    preset: CANVAS_PRESETS.splitHw,
    anchor: 'center',
    scaleDesign: 1,
  })
  void sceneScale

  // 整理 BaseChart 数据点
  const anodeLineData = chemistry.massHistory.map((p) => ({
    x: p.time,
    y: p.anodeDeltaM,
  }))

  const cathodeLineData = chemistry.massHistory.map((p) => ({
    x: p.time,
    y: p.cathodeDeltaM,
  }))

  const ionLineData = chemistry.ionHistory.map((p) => ({
    x: p.time,
    y: p.cMain,
  }))

  const neLineData = chemistry.ionHistory.map((p) => ({
    x: p.time,
    y: p.ne,
  }))

  return (
    <div className="w-full h-full flex flex-row overflow-hidden">
      {/* 左屏 280px 装置动画区 */}
      <div className="w-[280px] h-full shrink-0 border-r border-slate-200/80">
        <AnimationSvgCanvas containerRef={containerRef} transform={vp.transform}>
          <PrimaryCellScene
            chemistry={chemistry}
            canvasSize={canvasSize}
            cellType={cellType}
            electrolyteType={electrolyteType}
            loadType={loadType}
            current={current}
            time={time}
          />
        </AnimationSvgCanvas>
      </div>

      {/* 右侧 560px 图表展现区（两图表弹性平分，禁止手写固定高度） */}
      <div className="flex-1 h-full min-w-0 flex flex-col bg-white">
        <div className="flex-1 min-h-0 w-full p-2 border-b border-slate-200/60">
          <BaseChart
            title="两极电极质量变化 Δm - 时间"
            xDomain={[0, 10]}
            yDomain={[-20, 20]}
            xLabel="时间 t (s)"
            yLabel="质量变化 Δm (g)"
          >
            <ChartLine
              points={anodeLineData}
              color={CHEMISTRY_COLORS.anode}
              strokeWidth={2}
            />
            <ChartLine
              points={cathodeLineData}
              color={CHEMISTRY_COLORS.cathode}
              strokeWidth={2}
            />
            <ChartCursor
              x={time}
              dataPoints={[
                { y: chemistry.anodeDeltaM, label: `负极 Δm: ${chemistry.anodeDeltaM}g` },
                { y: chemistry.cathodeDeltaM, label: `正极 Δm: ${chemistry.cathodeDeltaM}g` },
              ]}
            />
          </BaseChart>
        </div>

        <div className="flex-1 min-h-0 w-full p-2">
          <BaseChart
            title="电子转移数 n(e⁻) & 关键反应物浓度 - 时间"
            xDomain={[0, 10]}
            yDomain={[0, 1.2]}
            xLabel="时间 t (s)"
            yLabel="浓度 (mol/L) / 电子 (mol)"
          >
            <ChartLine
              points={ionLineData}
              color={CHEMISTRY_COLORS.concentration}
              strokeWidth={2}
            />
            <ChartLine
              points={neLineData}
              color={CHEMISTRY_COLORS.electron}
              strokeWidth={2}
            />
            <ChartCursor
              x={time}
              dataPoints={[
                { y: chemistry.cMain, label: `浓度 c: ${chemistry.cMain}mol/L` },
                { y: chemistry.ne, label: `n(e⁻): ${chemistry.ne}mol` },
              ]}
            />
          </BaseChart>
        </div>
      </div>
    </div>
  )
}
