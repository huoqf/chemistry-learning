...
function useAnimationConfig() {
  ...
  const { setAnimationType, setParams, setTime, setIsPlaying } = useAnimationStore(
    useShallow((s) => ({
...
function usePlaybackLoop(
  ...
  const setEquilibriumReached = useAnimationStore((s) => s.setEquilibriumReached)
  ...
  useAnimationFrame(
    (deltaTime) => {
      ...
      // 化学平衡条件终止检测
      if (config?.stopCondition && config.stopCondition(params, t)) {
        setIsPlaying(false)
