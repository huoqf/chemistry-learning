...
export interface UseSceneScaleOptions {
  ...
  // ── center 模式 ──
  /** 设计坐标比例（design-unit / domain-unit），anchor='center' 时必填 */
  scaleDesign?: number
  /** @deprecated 用 scaleDesign */
  physicsScaleDesign?: number
  /** 非等比缩放 X 方向（可选，覆盖 scaleDesign） */
  scaleDesignX?: number
  /** @deprecated 用 scaleDesignX */
  physicsScaleDesignX?: number
  /** 非等比缩放 Y 方向（可选，覆盖 scaleDesign） */
  scaleDesignY?: number
  /** @deprecated 用 scaleDesignY */
  ...
  // ── viewport / design 模式 ──
  /** 场景域宽度，anchor='viewport'|'design' 时必填 */
  worldWidth?: number
  /** @deprecated 用 worldWidth */
  physicsWidth?: number
  /** 场景域高度，anchor='viewport'|'design' 时必填 */
  worldHeight?: number
  /** @deprecated 用 worldHeight */
...
export function useSceneScale(options: UseSceneScaleOptions): SceneScale {
  const {
    vp, preset, anchor,
    scaleDesign: scaleDesignNew, physicsScaleDesign: scaleDesignOld,
    scaleDesignX: scaleDesignXNew, physicsScaleDesignX: scaleDesignXOld,
    scaleDesignY: scaleDesignYNew, physicsScaleDesignY: scaleDesignYOld,
    ...
    worldWidth: worldWidthNew, physicsWidth: worldWidthOld,
    worldHeight: worldHeightNew, physicsHeight: worldHeightOld,
    refMagnitudes, maxVectorLength,
    centerLayout, intentionalNonUniformScale,
  } = options

  // 新名优先，旧名兜底
  const physicsScaleDesign = scaleDesignNew ?? scaleDesignOld
  const physicsScaleDesignX = scaleDesignXNew ?? scaleDesignXOld
  const physicsScaleDesignY = scaleDesignYNew ?? scaleDesignYOld
  const physicsWidth = worldWidthNew ?? worldWidthOld
  const physicsHeight = worldHeightNew ?? worldHeightOld
