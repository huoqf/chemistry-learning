...
const DB_NAME = 'chemistry-learning-db';
const STORE_NAME = 'chemistry-store';
const MAX_DB_SIZE = 200 * 1024 * 1024;

interface ChemistryDBSchema extends DBSchema {
  [STORE_NAME]: {
    key: string;
    value: unknown;
  };
...
let dbPromise: Promise<IDBPDatabase<ChemistryDBSchema>> | null = null;

async function getOrOpenDB(): Promise<IDBPDatabase<ChemistryDBSchema>> {
  if (!dbPromise) {
    dbPromise = openDB<ChemistryDBSchema>(DB_NAME, 1, {
      upgrade(db) {
