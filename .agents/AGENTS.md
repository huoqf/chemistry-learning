...
### 铁律 3C：左屏排版纯粹性与教学提示说明定位（避免教条）

- **标题纯粹**：选项卡片与模式切换按钮仅保留核心大标题，**严禁硬塞晦涩次级副标题**，**严禁堆砌装饰性 emoji 或小图标**（保持学术沉稳）。
- **教学提示归宿**：仅在需要交代反应前提（如强酸/加热）或启发思考时使用（避免教条），统一置于左屏底部独立 `LeftPanelSection`，结构化说明“实验条件、核心设问、观察指引”。
...
### 铁律 10：模块拆分领域内聚准则（严禁为拆而拆）

模块拆分的唯一目的在于**提高代码内聚度、化学可读性与长期可维护性**，绝对禁止为了迎合 linter/hook 行数限制而进行机械切断。

| 严禁行为（违规即作废） | 正确做法与领域设计原则 |
|---|---|
| ❌ 纯按行数机械腰斩（如 `dataA.ts`/`dataB.ts`/`part1.ts` 等无领域语义命名） | ✅ 必须按**化学业务领域边界**拆分（如 `cheatSheetData.ts` 速查表 vs `retrosynthesisModels.ts` 反应模型） |
| ❌ 将同一概念对硬生生拆分到不同文件（如把 L-乳酸放在基团文件，把 D-乳酸放在异构体文件） | ✅ **概念完整性闭环**：对映异构对、顺反异构对、双向绑定状态必须在同一模块完整内聚 |
| ❌ 为了单文件行数强行把高度内聚的单组件粉碎，引发严重的 props drilling 和上下文割裂 | ✅ **高内聚优先**：若单组件职责单一且拆分无收益，经评估在 `scripts/check-large-files.mjs` 中添加附带充分技术理由的白名单 |
| ❌ 拆分后产生循环引用或模糊中间层 | ✅ **单向依赖流**：各子模块职责单一，由统一入口文件（如 `index.ts` / `xxxData.ts`）集中组合导出 |
