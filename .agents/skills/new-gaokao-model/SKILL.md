...
### 4.2 右屏上下文严格强过滤
- **100% 母题强相关**：右屏的高考推断题眼与单基团精讲必须依据当前分子实际包含的基团（`groupCounts > 0`）进行严格过滤；
- **杜绝无关干扰**：严禁在选择特定母题（如阿司匹林）时在右屏展示完全不相关的题眼（如银镜反应、碳碳双键等）；
- **取消生硬 Tab**：右屏采用垂直单容器流，自上而下呈现【母题机理/方程式/避坑 ➔ 聚焦基团精讲 ➔ 本分子高考推断题眼】，天然顺畅滚动。
