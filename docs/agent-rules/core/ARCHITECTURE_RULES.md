...
### 3.3 模块拆分与领域内聚铁律（严禁为拆而拆）

模块拆分的唯一目的在于**提高代码内聚度、化学语义可读性与长期可维护性**，绝对禁止为了迎合代码行数限制进行无领域语义的机械切割。

1. **严禁机械字母拆分**：禁止出现 `xxxA.ts` / `xxxB.ts` / `xxxPart1.ts` 等纯按行数截断的无意义命名；拆分必须拥有明确的业务或学科领域命名（如 `retrosynthesisCheatSheet.ts` vs `retrosynthesisModels.ts`）。
2. **概念对完整性闭环**：属于同一概念对、对映异构体（如 L/D-乳酸）、顺反异构对或紧耦合状态机，必须在同一模块内聚，严禁人为割裂分置。
3. **高内聚优先于低行数**：对于职责高度单一的视觉呈现组件或数据字典，若强行拆分会引发剧烈的 props drilling 或逻辑断层，应优先在 `scripts/check-large-files.mjs` 中添加具有合理技术理由的白名单，严禁过度粉碎。
4. **单向依赖与统一入口**：拆分后的子模块由本领域的统一入口（如 `index.ts` / `xxxData.ts`）集中组合导出，上层消费方对底层拆分细节透明。
