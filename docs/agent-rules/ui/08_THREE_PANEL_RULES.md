...
### 2.5 左侧屏文案排版纯粹性与教学提示规范（避免教条）

#### 1. 标题与按钮文案保持纯粹干净（严禁在标题上乱搞）
- **职责聚焦**：选项卡片、模式切换按钮、分段控件仅承担**核心标识与动作选择**职能，标题文字必须干脆有力；
- **禁止副标题塞入生涩说明**：严禁在卡片大标题下方硬塞晦涩次级小字（如冷门化学名、长串分子式等无感说明）；
- **禁止滥用装饰图标/emoji**：严禁在标题或按钮前随意堆砌 emoji 或杂乱小图标（如 📚 🧩 📊 📑 🔍 等），确保学术界面的沉稳与专业。

#### 2. 教学提示说明的正确位置与结构（有必要时在下方结构化呈现）
- **使用场景（因地制宜，避免教条）**：仅在当前实验/母题体系存在特定反应前提（如强酸/加热）、或需要启发学生观察思考时使用；普通纯数值调节页面无需强行添加；
- **位置**：置于左屏底部，作为独立的 `LeftPanelSection title="教学思考与探究提示"`；
- **结构化三要素**：
  1. **实验与反应条件**：说明强酸/强碱、加热、常温中和、催化剂等关键物理化学前提；
  2. **核心设问与思考**：提出导向性问题（如断键位置、定量消耗为何不对称等）；
  3. **观察指引**：明确指引学生观察中屏微观/定量拆解，并在右屏核对标准高考方程式与避坑踩分点。
...
### 3.4 右屏内容与左屏选项强绑定铁律（严禁跨模式静态残留）

右屏的四大功能板块（化学量 `quantities`、核心公式 `formulas`、高考要点 `gaokaoPoints`、易错警示 `warnings`）必须与左屏的当前选项（`mode`、`reactionType`、`system`、`tab` 等）保持 **100% 动态纯函数同步**。

#### 1. 严禁无模式条件的单一大数组
- ❌ **禁止行为**：在右屏将所有题型或所有模式下的化学量、方程式和要点简单拼装成一个全局静态数组，导致切换到其他模式时无关内容持续显示。
- ✅ **正确做法**：采用 **模式字典映射模式 (Mode Dictionary Pattern)** 或 `useMemo` 分支过滤，根据左屏当前状态精准提供对应的知识元数据。

#### 2. 无效化学量物理条件守卫
- ❌ **禁止行为**：在无产气反应下渲染 `气体体积 V: 0 L`，在全液相或溶解反应下渲染 `电极增重: 0 g` 等无化学意义的无效占位。
- ✅ **正确做法**：仅在当前物理过程确实产生该变量时才将该条目推入 `quantities`，否则彻底从列表中剔除。

#### 3. 考点与警示专属化
- ❌ **禁止行为**：把其他题型的常考陷阱（如二次电池充电接线、络合反应）跨题型显示在无关模式中。
- ✅ **正确做法**：高考要点与易错警示必须 100% 针对当前左屏选中的具体实验情境。
